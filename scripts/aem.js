 ...
 */
import blocks from 'virtual:blocks:decorate';
import preload from 'virtual:blocks:preload';
    ...
    try {
        ...
        if (!window.hlx.rum) {
            ...
            rumSessionStorage.pages =
                (rumSessionStorage.pages ? rumSessionStorage.pages : 0) +
                1 +
...
function setup() {
    ...
    window.hlx.codeBasePath = import.meta.resolve('../').replace(/\/+$/, '');
    window.hlx.fragmentBasePath = window.hlx.codeBasePath;
        ...
        if (row.children) {
            ...
            if (cols[1]) {
                ...
                if (col.querySelector('a')) {
                    ...
                    if (as.length === 1) {
                        value = as[0].getAttribute('href');
                    } else {
                        value = as.map((a) => a.getAttribute('href'));
                    }
...
/**
 * Retrieves the href of canonical tag.
...
function createOptimizedPicture(
    ...
    alt = '',
    eager = false,
    breakpoints = [{ media: '(min-width: 600px)', width: '1000' }, { width: '550' }],
) {
...
function decorateSections(main) {
    main.querySelectorAll(':scope > div:not([data-section-status])').forEach((section) => {
        ...
        const meta = sectionMeta ? readBlockConfig(sectionMeta) : {};
        section.classList.add(meta.appearance || 'full-width');
        section.classList.add(`bg-${meta.background || 'none'}`);
        section.classList.add(`color-${meta.color || 'inherit'}`);
        section.classList.add(`padding-${meta.padding || 'dynamic'}`);
        section.classList.add(`align-${meta.align || 'dynamic'}`);
        if (sectionMeta) {
...
async function fetchPlaceholders(prefix = 'default') {
    ...
    if (!window.placeholders[prefix]) {
        window.placeholders[prefix] = new Promise((resolve) => {
            fetch(`${window.hlx.fragmentBasePath}${prefix === 'default' ? '' : prefix}/placeholders.json`)
                .then((resp) => {
                    if (resp.ok) {
                        return resp.json();
                    }
...
async function loadBlock(block) {
    ...
    if (status !== 'loading' && status !== 'loaded') {
        ...
        try {
            const blockLoader = blocks[blockName];
            if (!blockLoader) {
                throw new Error('Unknown block');
            }
            const preloadLoader = preload[blockName];
            const loaders = [blockLoader(), preloadLoader ? preloadLoader(block) : null];
            const [module, preloadData] = await Promise.all(loaders);
            if (module.default) {
                await module.default(block, preloadData);
            }
        } catch (error) {
            // eslint-disable-next-line no-console
